...
public class DbInitializer
{
    ...
    private static async Task SeedData(AuctionDbContext auctionDbContext)
    {
        await auctionDbContext.Database.MigrateAsync();

        if(await auctionDbContext.Auctions.AnyAsync())
        {
            Console.WriteLine("There is already data in the db, please clear before running the seeding");
            return;
